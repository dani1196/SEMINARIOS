...
\begin{frame}{Problema Inverso en Restauración de Imágenes}
   Se considera el siguiente problema variacional 
    ...
    donde:
    \begin{itemize}
        \item $\Omega$ representa el dominio de la imagen y $|\Omega|$ denota su cardinalidad.
        \item $\alpha>0$, $q\in (0,1)$, $0<\mu\ll \alpha$ son los parámetros del modelo. 
        \item El vector $z \in \mathbb{R}^{|\Omega|}$ representa la imagen dañada, mientras que $u\in\mathbb{R}^{|\Omega|}$ es la imagen reconstruida.
        \item $K \in \mathbb{R}^{|\Omega| \times |\Omega|}$ es una matriz que no anula al vector constante (por ejemplo, la matriz de desenfoque). $|(\nabla u)_{ij}|$ corresponde a la norma euclidiana de $(\nabla u)_{ij}\in \mathbb{R}^2.$
        \item $\lambda_{ij}>0\in \mathbb{R}^{|\Omega|}$ es el coeficiente de fidelidad y $\nabla$ es el operador gradiente discreto $\nabla=(\nabla_x\;\nabla_y).$
    \end{itemize}
\end{frame}
...
\begin{frame}{Regularización del término $TV^q$}
    ...
    \label{2.4}
    \end{equation}
    Dado que $f_\gamma$ es continuamente diferenciable, la ecuación de Euler-Lagrange está dada por 
    \begin{equation}
...
\begin{frame}{13. Diferenciabilidad Semismooth (Slanted Differentiability)}
    ...
    \begin{block}{Definición de Operador Semismooth}
        ...
        \begin{equation}
            \lim_{h \to 0} \frac{\parallel G(x+h) - G(x) - G^\circ(x+h)h \parallel}{\parallel h \parallel} = 0
        \end{equation}
    \end{block}
...
\begin{frame}{17. Regularización R Adaptativa}
    ...
    Donde:
    \begin{itemize}
        \item $R^k \in \mathbb{R}^{N \times N}$ es una matriz simétrica definida positiva elegida convenientemente como el operador elíptico estándar.
...
\begin{frame}{25. Efecto Estructural del Exponente Fraccionario $q$}
    \begin{itemize}
        ...
        \item \textbf{Caso $q \to 0.1$ (No Convexidad Extrema):} Promueve un contraste hiper-definido, pero vuelve el funcional extremadamente plano en los mínimos locales.
    \end{itemize}
\end{frame}
...
\begin{frame}{28. Relajación y la Envolvente Bipolar}
    ...
    \begin{itemize}
        ...
        \item Los autores demuestran el llamado \textbf{Efecto Bolza}, donde el ínfimo del problema original coincide con el mínimo del problema relajado.
    \end{itemize}
\end{frame}
...
\begin{frame}{29. Conclusiones y El Gran Aporte del Paper}
    ...
    \begin{enumerate}
        \item \textbf{Ruptura del Parafígma Lineal:} Demostró que los problemas no convexos no lineales no suaves pueden ser resueltos con la misma velocidad (o superior) que los modelos convexos clásicos si se explota adecuadamente la estructura matemática del Hessiano generalizado.
...
\begin{frame}[plain]
    \centering
    \Huge \color{matheblue} \textbf{¿Preguntas?}
    \vspace{0.4cm} \par
    \Large \color{emeraldteal} Muchas gracias por su atención en esta sesión técnica. \par
    \vspace{1.2cm}
    \footnotesize Maestría en Optimización Matemática \par
    \scriptsize Universidad de Investigación - Ingeniería Matemática \par
    \tiny Referencia: M. Hintermüller, T. Wu. \textit{Nonconvex TV-Models in Image Restoration}, SIAM J. Imaging Sciences, 2013.
\end{frame}
